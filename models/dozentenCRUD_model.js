...
const delete_dozent = async (req, res) => {
  const { id } = req.params;
  try {
    const kontaktDeleteAbfrage = "DELETE FROM kontakt_daten WHERE fk_dozent_id = $1";
    await pool.query(kontaktDeleteAbfrage, [id]);

    const dozentDeleteAbfrage = "DELETE FROM dozenten WHERE dozent_id =$1";
    await pool.query(dozentDeleteAbfrage, [id]);

    res.status(200).send("Dozent erfolgreich gelöscht!");
  } catch (error) {
